...
## 12) Marker → LME Dimension Mapping (Mapping-Matrix)

### 12.1 Ziel
Diese Matrix definiert **eindeutig**, wie `payload.markers[]` in **LME-Dimension-Deltas** übersetzt werden.

- **Event liefert:** Marker (`id`, `weight`)
- **Aggregator macht:** Marker → `DimensionDelta[]` → LME `apply(...)`

### 12.2 Dimension Registry (v1)
> Falls im Repo bereits `src/lib/lme/psyche-dimensions.ts` existiert, sollen diese IDs dort gespiegelt/aliast werden.
...
export type DimensionId =
  // Social
  | "dim.social.extroversion"
  | "dim.social.planning"
  | "dim.social.deep_talk"
  | "dim.social.dominance"
  | "dim.social.battery"
  ...
  | "dim.love.autonomy"
  | "dim.love.jealousy"
  | "dim.love.intimacy_pace"

  // Lifestyle
  | "dim.lifestyle.chronotype"
  | "dim.lifestyle.party"
  | "dim.lifestyle.stimulation"
  | "dim.lifestyle.routine"
  | "dim.lifestyle.order"
  | "dim.lifestyle.sleep_need"
  ...
  | "dim.skills.language"
  | "dim.skills.focus"
  | "dim.cognition.system_vs_story"

  // EQ
  | "dim.eq.empathy"
  | "dim.eq.self_regulation"
  | "dim.eq.sensitivity"
  ...
  // Astro (separat halten; Standard: nur schwacher Einfluss)
  | "dim.astro.fire"
  | "dim.astro.earth"
  | "dim.astro.air"
  | "dim.astro.water"
...
### 12.3 Übersetzungsregel (Marker → DimensionDelta)

**Marker-Weight Konvention:**
- `marker.weight` ist **immer 0..1** (Betrag)
- Richtung/Polung wird über Mapping (`sign`) bestimmt

```ts
type DimensionDelta = {
  dimensionId: DimensionId;
  delta: number;            // empfohlen: -1..+1 pro Event (geclamped)
  sourceMarkerId: string;
  confidence?: number;      // optional
};

type MarkerToDimensionRule = {
  dimensionId: DimensionId;
  sign: 1 | -1;
  scale?: number;           // default 1.0
  capAbs?: number;          // default 1.0
};

const baseScaleByVertical: Record<string, number> = {
  quiz: 1.0,
  horoscope: 0.2,           // Horoskop = Flavor/soft influence
  character: 1.0,
  future: 1.0
};

function markerToDelta(marker: Marker, rule: MarkerToDimensionRule, vertical: string): DimensionDelta {
  const scale = (rule.scale ?? 1.0) * (baseScaleByVertical[vertical] ?? 1.0);
  const raw = rule.sign * marker.weight * scale;
  const cap = rule.capAbs ?? 1.0;
  const clamped = Math.max(-cap, Math.min(cap, raw));
  return {
    dimensionId: rule.dimensionId,
    delta: clamped,
    sourceMarkerId: marker.id,
    confidence: marker.evidence?.confidence
  };
}
```

**Zusammenführung, wenn mehrere Marker dieselbe Dimension treffen:**
- Summe bilden, danach clamp auf `[-1..+1]` pro Event (oder `capAbs`)

### 12.4 Mapping-Matrix (konkret)

```ts
export const MARKER_TO_DIMENSION: Record<string, MarkerToDimensionRule> = {
  // Social
  "marker.social.extroversion": { dimensionId: "dim.social.extroversion", sign: +1 },
  "marker.social.introversion": { dimensionId: "dim.social.extroversion", sign: -1 },

  "marker.social.planning": { dimensionId: "dim.social.planning", sign: +1 },
  "marker.social.spontaneity": { dimensionId: "dim.social.planning", sign: -1 },

  "marker.social.deep_talk": { dimensionId: "dim.social.deep_talk", sign: +1 },
  "marker.social.smalltalk": { dimensionId: "dim.social.deep_talk", sign: -1 },

  "marker.social.dominance": { dimensionId: "dim.social.dominance", sign: +1 },
  "marker.social.harmony": { dimensionId: "dim.social.dominance", sign: -1 },

  // optional: falls Battery als Marker kommt
  "marker.social.battery": { dimensionId: "dim.social.battery", sign: +1 },

  // Love
  "marker.love.attention_need": { dimensionId: "dim.love.attention", sign: +1 },
  "marker.love.intimacy_need": { dimensionId: "dim.love.intimacy", sign: +1 },

  "marker.love.autonomy_need": { dimensionId: "dim.love.autonomy", sign: +1 },
  "marker.love.fusion_need": { dimensionId: "dim.love.autonomy", sign: -1 },

  "marker.love.jealousy": { dimensionId: "dim.love.jealousy", sign: +1 },

  // Lifestyle
  "marker.lifestyle.night_owl": { dimensionId: "dim.lifestyle.chronotype", sign: -1 },
  "marker.lifestyle.early_bird": { dimensionId: "dim.lifestyle.chronotype", sign: +1 },

  "marker.lifestyle.party": { dimensionId: "dim.lifestyle.party", sign: +1 },
  "marker.lifestyle.calm": { dimensionId: "dim.lifestyle.stimulation", sign: -1 },

  // Interests
  "marker.interest.gaming": { dimensionId: "dim.interest.gaming", sign: +1 },
  "marker.interest.nature": { dimensionId: "dim.interest.nature", sign: +1 },
  "marker.interest.travel": { dimensionId: "dim.interest.travel", sign: +1 },

  // Skills / Cognition
  "marker.skills.math": { dimensionId: "dim.skills.math", sign: +1 },
  "marker.skills.language": { dimensionId: "dim.skills.language", sign: +1 },
  "marker.skills.focus": { dimensionId: "dim.skills.focus", sign: +1 },

  "marker.cognition.system_thinking": { dimensionId: "dim.cognition.system_vs_story", sign: +1 },
  "marker.cognition.story_thinking": { dimensionId: "dim.cognition.system_vs_story", sign: -1 },

  // EQ
  "marker.eq.empathy": { dimensionId: "dim.eq.empathy", sign: +1 },
  "marker.eq.self_regulation": { dimensionId: "dim.eq.self_regulation", sign: +1 },
  "marker.eq.sensitivity_high": { dimensionId: "dim.eq.sensitivity", sign: +1 },

  // Aura
  "marker.aura.charisma": { dimensionId: "dim.aura.charisma", sign: +1 },
  "marker.aura.leadership": { dimensionId: "dim.aura.leadership", sign: +1 },

  // Astro (Flavor; wird ohnehin zusätzlich über payload.astro gerendert)
  "marker.astro.element.fire": { dimensionId: "dim.astro.fire", sign: +1, scale: 0.5 },
  "marker.astro.element.earth": { dimensionId: "dim.astro.earth", sign: +1, scale: 0.5 },
  "marker.astro.element.air": { dimensionId: "dim.astro.air", sign: +1, scale: 0.5 },
  "marker.astro.element.water": { dimensionId: "dim.astro.water", sign: +1, scale: 0.5 },

  "marker.astro.modality.cardinal": { dimensionId: "dim.astro.cardinal", sign: +1, scale: 0.5 },
  "marker.astro.modality.fixed": { dimensionId: "dim.astro.fixed", sign: +1, scale: 0.5 },
  "marker.astro.modality.mutable": { dimensionId: "dim.astro.mutable", sign: +1, scale: 0.5 }
};
```

### 12.5 Trait → Marker Standard-Mapping (für Quiz-Agenten)
Damit Quiz-Agenten nicht jedes Mal neu erfinden:

- Für **Balance-Traits** (z.B. Introversion↔Extroversion):
  - `w = abs((score - 50) / 50)`
  - Marker-ID je nach Seite, Weight = `w`

Beispiel:
- `trait.social.introversion` (Score 1..100)
  - score > 50 ⇒ `marker.social.introversion` weight = (score-50)/50
  - score < 50 ⇒ `marker.social.extroversion` weight = (50-score)/50

### 12.6 Validierungsregel (DoD für Agenten)
Ein Modul gilt als kompatibel, wenn:
1. alle Marker-IDs in `MARKER_TO_DIMENSION` existieren
2. `weight` ist 0..1
3. Aggregator erzeugt nur DimensionIds aus `DimensionId`
4. Ein Test: `ingestContribution(event)` ändert deterministisch den PsycheState und das Character Sheet zeigt den Block als "filled" an.
