...
export function BlumenwesenQuiz() {
  ...
  const [scores, setScores] = useState<DimensionScores>({ d1: 0, d2: 0, d3: 0 });
      ...
      <div className="min-h-[600px] rounded-3xl overflow-hidden bg-gradient-to-br from-[#1A3C2F] to-[#0D1F18] p-6 flex flex-col items-center justify-center">
        <div className="bg-[#2D5A4C]/30 backdrop-blur-md rounded-3xl p-8 max-w-md w-full border border-[#A8D5BA]/20 shadow-2xl">
          <div className="text-center mb-6">
            ...
            <div className="text-6xl mb-4 p-4 bg-white/5 rounded-full inline-block">{/* @ts-ignore */ result.emoji || "🌸"}</div>
          ...
          <div className="space-y-3">
            {currentQ.options.map(option => (
              <button
                ...
                className="w-full text-left p-4 rounded-xl border border-[#C0D0C8] hover:border-[#2D5A4C] hover:bg-[#F0F5F2] transition-all text-[#2D5A4C] font-medium"
              >
                {option.text}
              </button>
            ))}
